  ...
  <div>
    ...
    <Modal 
      ...
      :mask-closable="false">
        ...
        <Form ref="modalForm" :model="modalDataForm" :rules="modalDataRule" inline>
          <!-- 企业生产数据 -->
          <div v-if="modalIndex === '1'">
            <div class="title">基本信息</div>
            <FormItem label="年度期末运营车辆数" prop="carCountYearUse" :label-width="160">
              <InputNumber v-model="modalDataForm.carCountYearUse" controls-outside :min="0" />
            </FormItem>
            <FormItem label="年度期末在册车辆数" prop="carCountYearOn" :label-width="160">
              <InputNumber v-model="modalDataForm.carCountYearOn" controls-outside :min="0" />
            </FormItem>
            <FormItem label="投运时间(可按线路填写)" prop="useTime" :label-width="180">
              <DatePicker v-model="modalDataForm.useTime" type="date" placeholder="投运时间" style="width: 144px" />
            </FormItem>

            <div class="title">生产信息</div>
            <FormItem label="总行程(万公里)" prop="totalLine" :label-width="160">
              <InputNumber v-model="modalDataForm.totalLine" controls-outside :min="0" />
            </FormItem>
            <FormItem label="客运量(万人次)" prop="totalPerson" :label-width="160">
              <InputNumber v-model="modalDataForm.totalPerson" controls-outside :min="0" />
            </FormItem>
            <FormItem label="旅客周转量(万人公里)" prop="totalTurnover" :label-width="180">
              <InputNumber v-model="modalDataForm.totalTurnover" controls-outside :min="0" />
            </FormItem>
...
export default {
  ...
  data(){
    return {
      ...
      modalDataRule : { // modal - 数据校验
        carCountYearUse : [{ required: true, message: '年度期末运营车辆数不能为空', trigger: 'blur' }],
        carCountYearOn : [{ required: true, message: '年度期末在册车辆数不能为空', trigger: 'blur' }],
        useTime : [{ required: true, message: '投运时间不能为空', trigger: 'blur' }],
        totalLine : [{ required: true, message: '总行程不能为空', trigger: 'blur' }],
        totalPerson : [{ required: true, message: '客运量不能为空', trigger: 'blur' }],
        totalTurnover : [{ required: true, message: '旅客周转量不能为空', trigger: 'blur' }],
      },
  ...
  okHandle(){
    ...
    SaveYearDataInput(){
      let params = Object.assign({}, {
        ...
        createTime : '', // 创建时间
        delFlag : '', // 删除标志
        energyBelongType : '', // 类型（企业生产消耗、辅助、附属设施消耗
        energyTypeId : '', // 能源种类
        energyTypeName : '', // 能源种类名称
        energyUnit : '', // 燃料单位
        enterpriseId : '', // 企业编号
        id : '', // 对应运营信息或附属辅助设施
        operationInfoId : '', // 对应运营信息或附属辅助设施
        quarterYear : '', // 年份季度
        state : '', // 状态
        updateBy : '', // 更新者
        updateTime : '' // 更新时间
